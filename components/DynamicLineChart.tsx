  ...
  const createSegments = (data: number[], isTemp: boolean) => {
    ...
    for (let i = 0; i < data.length - 1; i++) {
      ...
      if (!thresholds || (!crossesMin && !crossesMax)) {
        ...
        segments.push(
          <Line key={`${isTemp ? 'temp' : 'hum'}-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth={2} />
        );
      } else {
    ...
    <View>
      <Svg width={width} height={height}>
        ...
        {yTicks.map((tick, i) => {
          ...
          return (
            <G key={`ytick-${i}`}>
              <Line x1={padding.left - 5} y1={y} x2={padding.left} y2={y} stroke="#666" strokeWidth={1} />
        ...
        {xTicks.map((tickIndex, i) => {
          ...
          const time = new Date(packets[tickIndex].time * 1000).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
          return (
            <G key={`xtick-${i}`}>
              <Line x1={x} y1={height - padding.bottom} x2={x} y2={height - padding.bottom + 5} stroke="#666" strokeWidth={1} />
              <SvgText x={x} y={height - padding.bottom + 18} fontSize={10} fill="#666" textAnchor="middle">
                {time}
              </SvgText>
            </G>
          );
        ...
        {thresholds && (
          <>
            <Line x1={padding.left} y1={getY(thresholds.tempMin)} x2={width - padding.right} y2={getY(thresholds.tempMin)} stroke="#EF4444" strokeWidth={1} strokeDasharray="5,5" opacity={0.5} />
            <Line x1={padding.left} y1={getY(thresholds.tempMax)} x2={width - padding.right} y2={getY(thresholds.tempMax)} stroke="#EF4444" strokeWidth={1} strokeDasharray="5,5" opacity={0.5} />
            <Line x1={padding.left} y1={getY(thresholds.humMin)} x2={width - padding.right} y2={getY(thresholds.humMin)} stroke="#F97316" strokeWidth={1} strokeDasharray="5,5" opacity={0.5} />
            <Line x1={padding.left} y1={getY(thresholds.humMax)} x2={width - padding.right} y2={getY(thresholds.humMax)} stroke="#F97316" strokeWidth={1} strokeDasharray="5,5" opacity={0.5} />
          </>
        )}
